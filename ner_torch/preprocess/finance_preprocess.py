...
for t in list(train_df.text):
    additional_chars.update(re.findall(u'[^\u4e00-\u9fa5a-zA-Z0-9\*]', str(t)))   #第一个字符是取反，\u4e00-\u9fa5a匹配中文字符，a-z A-Z 0-9
...
print('-----数据清洗-------')
train_df.to_csv(data_dir + 'new_train_df.csv', encoding='utf8')

# 切分训练集，分成训练集和验证集
print('Train Set Size:', train_df.shape)
new_dev_df = train_df[2200:  3000]
new_test_df = train_df[3000: 3400 ]
frames = [train_df[:2200], train_df[3400:]]
...
def _cut(sentence):
    ...
    sen = []
    for i in sentence:
        if i in ['。', '！', '？', '?'] and len(sen) != 0:  #以这些标点符号来划分句子，这里没有逗号，分号
            sen.append(i)
...
def cut_train_and_dev_set(text_list, label_list):
    cut_text_list = []     # 由几句话组成的当前的这行文本
    cut_index_list = []    # 统计有几句话组成当前的文本
    cut_label_list = []    # 当前这行文本的标签
    for i, text in enumerate(text_list):
        if label_list[i] != '':            # 只要标签不为空
            text_label_list = label_list[i].split(';')  # 获取该条数据的实体列表
            temp_cut_text_list = []
            temp_cut_label_list = []
            text_agg = ''
            if len(text) < len_treshold:         # 如果小于设定的文本长度
                ...
                for sentence in sentence_list:
                    if len(text_agg) + len(sentence) < len_treshold: #将句子依次组装，直到长度超过512的临界值
                        text_agg += sentence
                    else:
...
test_label_list = new_test_df['unknownEntities'].tolist()
...
test_cut_text_list, test_cut_index_list, test_cut_label_list = cut_train_and_dev_set(test_text_list, test_label_list)
...
flag = True
for i, text in enumerate(test_cut_text_list):
    label_list = test_cut_label_list[i].split(';')
    for li in label_list:
        if li not in text:
            print(i)
            print(li)
...
test_dict = {'text': test_cut_text_list, 'unknownEntities': test_cut_label_list}
test_df = pd.DataFrame(test_dict)

train_df.to_csv(data_dir + 'new_train_set.csv', encoding='utf8')
dev_df.to_csv(data_dir + 'new_dev_set.csv', encoding='utf8')
test_df.to_csv(data_dir + 'new_test_set.csv', encoding='utf8')

print('训练集:', train_df.shape)
print('验证集:', dev_df.shape)
print('测试集:', test_df.shape)

# 构造训练集、验证集与测试集，制作成NER数据格式，方法是复制当前字符串，然后把标签进行替换，然后生成那种每列一个字和对应的标签的格式。
